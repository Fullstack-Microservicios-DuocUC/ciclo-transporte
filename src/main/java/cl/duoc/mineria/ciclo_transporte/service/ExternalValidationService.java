...
public class ExternalValidationService {
    ...
    public boolean verificarCamionActivo(Long camionId) {
        try {
            ...
            // Timeout, conexión rechazada, 5xx, etc: el servicio no puede confirmar ->
    ...
    public boolean verificarPaleroAutorizado(Long paleroId) {
        try {
            // Invoca al endpoint especializado que valida la existencia y el rol
            // OPERADOR_PALA
    ...
    public boolean verificarTurnoExiste(Long turnoId) {
        try {
            Boolean existe = webClient.get()
                    .uri("http://turnos/api/v1/turnos/existe/" + turnoId)
                    .retrieve()
                    .bodyToMono(Boolean.class)
                    .block();
            return existe != null && existe;
            ...
            throw new ServicioExternoNoDisponibleException(
                    "No se pudo validar el turno " + turnoId + ": " + e.getMessage());
        }
    }
}